...
<template>
  <div class="space-y-8 max-w-5xl mx-auto relative pb-16">
    ...
    <div v-if="fetchError"
      ...
      <p class="text-sm flex-grow">{{ fetchError }}</p>
      <button @click="loadDNA"
    ...
    <div v-else-if="isLoading" class="space-y-6 animate-pulse">
      <div class="h-36 rounded-3xl bg-white/[0.03] border border-white/[0.04]" />
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div v-for="i in 4" :key="i" class="h-28 rounded-2xl bg-white/[0.03] border border-white/[0.04]" />
      </div>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div v-for="i in 4" :key="i" class="h-56 rounded-3xl bg-white/[0.03] border border-white/[0.04]" />
      </div>
    </div>
    ...
    <div v-else class="space-y-8">
      ...
      <div class="relative overflow-hidden rounded-3xl border border-white/[0.06] bg-[#0e0e0e] p-8">
        <!-- decorative rings -->
        ...
        <div class="absolute -bottom-16 -left-16 w-64 h-64 rounded-full border border-[#7C6FFF]/10 pointer-events-none" />
        <div class="absolute -bottom-8  -left-8  w-48 h-48 rounded-full border border-[#7C6FFF]/10 pointer-events-none" />
        ...
        <div class="relative z-10 flex flex-col md:flex-row md:items-center gap-8">
          ...
          <div class="flex-shrink-0 flex items-center justify-center">
            <div class="relative w-24 h-24">
              <!-- outer spin ring -->
              <div class="absolute inset-0 rounded-full border-2 border-dashed border-brand/30 animate-spin [animation-duration:12s]" />
              <!-- mid ring -->
              <div class="absolute inset-3 rounded-full border border-[#7C6FFF]/40 animate-spin [animation-duration:8s] [animation-direction:reverse]" />
              <!-- core -->
              <div class="absolute inset-6 rounded-full bg-gradient-to-br from-brand/30 to-[#7C6FFF]/30 flex items-center justify-center border border-brand/30">
          ...
          <!-- Summary text -->
          <div class="flex-1">
            <div class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border border-brand/25 bg-brand/10 text-xs font-semibold text-brand uppercase tracking-wider mb-3">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div v-for="card in summaryCards" :key="card.label"
          class="group bg-[#111111] border border-white/[0.06] rounded-2xl p-5 flex flex-col gap-3 hover:border-white/[0.12] transition-all duration-200">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <!-- Format Affinity -->
        <div class="bg-[#111111] border border-white/[0.06] rounded-3xl p-6 hover:border-white/[0.1] transition-all">
      ...
      <!-- DNA Strand Footer -->
      <div class="rounded-2xl border border-white/[0.05] bg-[#0c0c0c] px-6 py-4 flex items-center gap-4">
        <span class="material-symbols-outlined text-gray-600 text-xl">info</span>
        <p class="text-xs text-gray-500 leading-relaxed">
          Content DNA is recalculated every 24 hours based on the last 90 days of performance. Scores reflect normalised engagement ratios, not raw view counts.
        </p>
      </div>
